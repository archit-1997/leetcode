...
public:
    ...
    void makeIsland(int x,int y,vector<vector<int>> &grid,vector<int> &size,vector<int> &par){
        int r=grid.size(),c=grid[0].size();
    ...
    int largestIsland(vector<vector<int>>& grid) {
        ...
        //now we have to traverse over each 0, temporarily make it 1 and count the size of the max island
        for(int i=0;i<r;i++){
            for(int j=0;j<c;j++){
                if(grid[i][j]==0){
